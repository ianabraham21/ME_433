package com.example.ianabraham.a433_camera;
...
//            startY = startY + j;
            ...
            // (array name, offset inside array, stride (size of row), start x, start y, num pixels to read per row, num rows to read)
            ...
            for (int i = 0; i < bmp.getWidth(); i++) {
                ...
                if (255  - (red(pixels[i]) + green(pixels[i])) > thresh) {
                    thresholdedPixels[i] = 255;
                    // this is giving me a bug, rgb is not a defied function
...
//            mTextView.setText("Threshold " + thresh);
            prevtime = nowtime;
        }
    }
}